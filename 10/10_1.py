...
def trailhead_score(initial_pos: Vector2, mountain: Map2) -> int:
    if mountain[initial_pos] != '0':
        return 0
    res = trailhead_score_rec(initial_pos, mountain, -1, set())
    return res

def trailhead_score_rec(pos: Vector2, mountain: Map2, last_val: int, already_visited: set[Vector2]) -> int:
    ...
    if curr_val != last_val + 1:
        ...
        already_visited.add(pos)
        return sum([trailhead_score_rec(pos + neigh_dir, mountain, curr_val, already_visited) for neigh_dir in NEIGH_DIRS])
